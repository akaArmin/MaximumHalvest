...
def bid(ticker, start, end):
    """Geldkurs (Verkauf): Close minus halber typischer Spread (~0.05%)"""
    close = download_close(ticker, start, end)
    return close * (1 - 0.0005)

def ask(ticker, start, end):
    """Briefkurs (Kauf): Close plus halber typischer Spread (~0.05%)"""
    close = download_close(ticker, start, end)
    return close * (1 + 0.0005)

o    = bid(SELL_TICKER, start, end)   # O verkaufen  → Geldkurs
udiv = ask(BUY_TICKER,  start, end)   # UDIV kaufen  → Briefkurs
fx   = download_close(FX_TICKER, start, end)
